...
st.markdown("<div style='font-size:0.72rem;color:#9ca3af;margin:2px 0 10px;'>👈 Tip: swipe from the left edge or tap the top-left arrow to open full chat history in the sidebar</div>", unsafe_allow_html=True)
...
if not (st.session_state.saved_api_key or "").strip():
    with st.expander("🔑 Enter your Groq API Key here (sidebar not required)", expanded=True):
        ...
        if mobile_api_key and mobile_api_key != st.session_state.saved_api_key:
            st.session_state.saved_api_key = mobile_api_key
            save_to_storage()
            st.rerun()
...
if chat is None:
    ...
    if prompt:
        ...
        if images:
            ...
            model_name = "openai/gpt-oss-120b"  # current Groq general model (llama3-8b-8192 was retired)
                ...
                try:
                    ...
                    if "401" in err or "invalid_api_key" in err.lower():
                        ...
                        try:
                            ...
                            resp2 = client.chat.completions.create(model="openai/gpt-oss-20b",messages=history,max_tokens=2048,temperature=0.7)
