...
def clean_reply(text):
    """Some reasoning models (like Qwen) include their internal <think>...</think>
    reasoning trace before the real answer. Strip it so users only see the answer."""
    if not text:
        return text
...
def web_search(query, max_results=4):
    import time
    # DuckDuckGo's free search endpoint is well known to rate-limit shared
    # cloud IPs (like Streamlit Cloud's). Trying a couple of different
    # backends with a short pause between them recovers most of the time.
...
with st.sidebar:
    ...
    st.divider()
    st.divider()
    st.markdown("<div style='font-size:0.72rem;color:#9ca3af;margin-bottom:4px;'>💾 Backup (extra safety)</div>", unsafe_allow_html=True)
    backup_data = json.dumps({
        "chats": st.session_state.chats,
        "current_id": st.session_state.current_id,
        "long_term_memory": st.session_state.long_term_memory
    }, ensure_ascii=False, indent=2)
    st.download_button("📥 Download backup", data=backup_data, file_name="shourya_backup.json", mime="application/json", use_container_width=True, key="backup_dl")
    restore_file = st.file_uploader("📤 Restore from backup", type=["json"], key="restore_up", label_visibility="collapsed")
    if restore_file is not None:
        try:
            restored = json.loads(restore_file.read().decode("utf-8"))
            if restored.get("chats"):
                st.session_state.chats.update(restored["chats"])
            if restored.get("long_term_memory"):
                st.session_state.long_term_memory = restored["long_term_memory"]
            if restored.get("current_id") and restored["current_id"] in st.session_state.chats:
                st.session_state.current_id = restored["current_id"]
            save_to_storage(); save_memory()
            st.success("✅ Restored!")
            st.rerun()
        except Exception:
...
# Mobile-safe top bar — works even if the sidebar is collapsed/hidden
top1, top2 = st.columns([3,2])
with top1:
    st.markdown("<div class='new-chat-wrap'>", unsafe_allow_html=True)
    if st.button("✏️  New Chat", use_container_width=True, key="topbar_new_chat"):
        create_new_chat(); save_to_storage(); st.rerun()
    st.markdown("</div>", unsafe_allow_html=True)
with top2:
    mode_list2 = list(MODES.keys())
    default_mode2 = get_chat()["mode"] if get_chat() else mode_list2[-1]
    def _sync_from_topbar():
        new_mode = st.session_state.get("topbar_mode")
        if get_chat() and new_mode:
            st.session_state.chats[st.session_state.current_id]["mode"] = new_mode
            st.session_state["sidebar_mode"] = new_mode
    top_mode = st.selectbox("", mode_list2, index=mode_list2.index(default_mode2), label_visibility="collapsed", key="topbar_mode", on_change=_sync_from_topbar)

# Claude-style chat history — always visible here, no sidebar needed
with st.expander(f"🗂️ Chat History ({len(st.session_state.chats)})", expanded=False):
    if st.session_state.chats:
        sorted_chats2 = sorted(st.session_state.chats.values(), key=lambda x: x["created"], reverse=True)
        for chat2 in sorted_chats2:
            is_active2 = chat2["id"] == st.session_state.current_id
            hc1, hc2 = st.columns([5,1])
            with hc1:
                label2 = ("▶  " if is_active2 else "💬  ") + chat2["title"]
                if st.button(label2, key=f"mhist_open_{chat2['id']}", use_container_width=True):
                    st.session_state.current_id = chat2["id"]; save_to_storage(); st.rerun()
            with hc2:
                if st.button("✕", key=f"mhist_del_{chat2['id']}"):
                    del st.session_state.chats[chat2["id"]]
                    if st.session_state.current_id == chat2["id"]: st.session_state.current_id = None
                    save_to_storage(); st.rerun()
    else:
        st.caption("No past chats yet — start one above!")

with st.expander("🧠 Long-Term Memory", expanded=False):
    st.caption("Facts Shourya remembers in every chat, permanently.")
    mem_input2 = st.text_area(
        "", value=st.session_state.long_term_memory,
        placeholder="e.g. I prefer Python. I'm building a food delivery app.",
        label_visibility="collapsed", height=90, key="mem_field_main"
    )
    if mem_input2 != st.session_state.long_term_memory:
        st.session_state.long_term_memory = mem_input2
        save_memory()

with st.expander("💾 Backup & Restore (protect your data)", expanded=False):
    st.caption("Download a backup file anytime. If your chats/memory ever disappear, restore from this file.")
    backup_data2 = json.dumps({
        "chats": st.session_state.chats,
        "current_id": st.session_state.current_id,
        "long_term_memory": st.session_state.long_term_memory
    }, ensure_ascii=False, indent=2)
    st.download_button("📥 Download backup", data=backup_data2, file_name="shourya_backup.json", mime="application/json", use_container_width=True, key="backup_dl_main")
    restore_file2 = st.file_uploader("📤 Restore from backup file", type=["json"], key="restore_up_main")
    if restore_file2 is not None:
        try:
            restored2 = json.loads(restore_file2.read().decode("utf-8"))
            if restored2.get("chats"):
                st.session_state.chats.update(restored2["chats"])
            if restored2.get("long_term_memory"):
                st.session_state.long_term_memory = restored2["long_term_memory"]
            if restored2.get("current_id") and restored2["current_id"] in st.session_state.chats:
                st.session_state.current_id = restored2["current_id"]
            save_to_storage(); save_memory()
            st.success("✅ Restored!")
            st.rerun()
        except Exception:
...
# Mobile-safe API key box — shown here too in case sidebar is unreachable
if not (st.session_state.saved_api_key or "").strip():
    with st.expander("🔑 Enter your Groq API Key here (sidebar not required)", expanded=True):
        mobile_api_key = st.text_input(
            "", type="password", placeholder="gsk_... (FREE at console.groq.com)",
            label_visibility="collapsed", key="mobile_api_key_field"
        )
        if mobile_api_key and mobile_api_key != st.session_state.saved_api_key:
            st.session_state.saved_api_key = clean_api_key(mobile_api_key)
            save_to_storage()
            st.rerun()
...
if chat is None:
    ...
    if st.session_state.show_uploader:
        new_files = st.file_uploader(
            ...
            type=["png","jpg","jpeg","gif","webp","pdf","docx","py","js","ts","tsx","jsx","html","css","txt","csv","json","md","cpp","c","java","rb","go","php","swift","kt","rs"],
    ...
    # ════════════════════════════════════
    if prompt:
        api_key = clean_api_key(st.session_state.saved_api_key or "")
        if not api_key:
        ...
        full_text_parts = []
        if search_context:
            full_text_parts.append(f"[REFERENCE INFORMATION found online about '{prompt}']:\n{search_context}\n\nUse this reference information to give a current, accurate answer.")
        if file_text:
        ...
        if images:
            ...
            model_name = "qwen/qwen3.6-27b"  # current Groq vision/multimodal model (llama-4-scout was retired)
        else:
            full_text_parts.append(prompt)
            api_content = "\n\n".join(full_text_parts)
            model_name = "openai/gpt-oss-20b"  # production-tier free model with much higher free-tier limits than the qwen preview model
        ...
        with st.chat_message("assistant",avatar="🤖"):
            with st.spinner("Shourya is thinking..."):
                try:
                    ...
                    system_prompt = MODES[cur_mode]
                    mem = st.session_state.get("long_term_memory","").strip()
                    if mem:
                        system_prompt += f"\n\nLONG-TERM MEMORY (facts to remember about this user across all chats):\n{mem}"
                    ...
                    elif "tool_use_failed" in err.lower() or "tool choice is none" in err.lower():
                        # Known Groq bug: some models fire their own built-in tool
                        # unexpectedly. Retry once on a model without that behavior.
                        try:
                            resp3 = client.chat.completions.create(model="qwen/qwen3.6-27b",messages=history,max_tokens=2048,temperature=0.7)
                            reply3 = clean_reply(resp3.choices[0].message.content)
                            st.markdown(reply3)
                            st.session_state.chats[cid]["messages"].append({"role":"assistant","content":reply3,"files":[]})
                            st.session_state.pending_files=[]
                            save_to_storage()
                            st.rerun()
                        except Exception as e3:
                            st.error(f"❌ Error: {str(e3)[:300]}")
                    elif "model" in err.lower() and ("not found" in err.lower() or "404" in err):
                        try:
                            history[-1]["content"] = (prompt + ("\n\n" + search_context if search_context else ""))
                            resp2 = client.chat.completions.create(model="qwen/qwen3.6-27b",messages=history,max_tokens=2048,temperature=0.7)
                            reply2 = clean_reply(resp2.choices[0].message.content)
                            st.markdown(reply2)
