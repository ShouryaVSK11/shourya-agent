...
with st.sidebar:
    ...
    mode = st.selectbox("", mode_list, index=mode_list.index(default_mode), label_visibility="collapsed", key="sidebar_mode")
    if get_chat() and mode != get_chat()["mode"]:
        st.session_state.chats[st.session_state.current_id]["mode"] = mode
        st.session_state["topbar_mode"] = mode  # keep the top-bar selector in sync
    st.divider()
...
with top2:
    ...
    top_mode = st.selectbox("", mode_list2, index=mode_list2.index(default_mode2), label_visibility="collapsed", key="topbar_mode")
    if get_chat() and top_mode != get_chat()["mode"]:
        st.session_state.chats[st.session_state.current_id]["mode"] = top_mode
        st.session_state["sidebar_mode"] = top_mode  # keep the sidebar selector in sync
